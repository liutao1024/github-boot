...
//		alert("index.js-Error")
	 ...
	 * type:POST-add,PUT-update,GET-query,DELTE,按照Restful标准,默认get 
	 * successfn:成功回调函数 
	 * errorfn:失败回调函数 
	 ...
	 * 
	 */
	var _ajaxSun = function(url, data, type, successfn, errorfn, dataType, async) {
					...
					if(d.retMsg.substr(0,12)=='[Perm.E0016]'){
						bootbox.confirm("当前柜员对本交易操作权限不够,需要授权是否需要进行授权?", function (result) {  
							if(result) {  
								...
								$("#check_save").attr("disabled",false);					
								$("#check_save",$("#index_checkModal")).bind('click',function(){
//									e.preventDefault();
									$("#check_save").attr("disabled",true);		
									...
									//验证复核密码
									Sunline.ajaxRouter("recheck/uschck", checkData, "POST", function(ret){
										if(ret.retCode == "0000"){
	...
	var _getDict = function(type, path, id, text) {
		...
		var dictId="dictId";
		var dictUrl= _basePath+"/dict";//var dictUrl= _basePath+"/rest/dict";
		if(path!=null){
			dictUrl+=path;
			dictName=text;
			dictId=id;
	...
	var _ajaxPath = function(url, node) {
		...
		if (_isDemo) {
			...
			url = _basePath + "/" + url;//url = _basePath + "/rest/" + url;
		}
	...
	var _localPath =function(url){
		return  window.location.href = _basePath + "/" + url;	
//		return  window.location.href = _basePath + "/path/" + url;	
	...
	return {
		 ...
		 */
		ajaxRouter : function(url, data, type, successfn, errorfn, dataType, async) {
