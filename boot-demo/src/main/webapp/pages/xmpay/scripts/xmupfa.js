...
*/	
	...
	var readFile = function(){
		...
		$('#submit').click(function(){
			...
			$("#myModal").modal('show');
			Sunline.ajaxRouter(
	        	"xmpays/getXmpayChinaPayDkRecord", 
	        	 input,
	        	"POST",
	            function(redata){
	...
	var handlerTable = function(){
			var url = Sunline.ajaxPath("xmpays/qryXmpayChinaPayDkRecord");
...
//				}
				...
				for (var i=0;i<rows.length;i++){
					...
					var acctnm = $(row[2]).text();
					var custac = $(row[3]).text();
					var fronsq = $(row[4]).text();
					var frondt = $(row[5]).text();
					var tranAm = $(row[6]).text();
					var lncfno = $(row[7]).text();
					var checkStatus = $(row[8]).text();
					var rowData = {};
					rowData.checkDate = checkDate;
					rowData.acctnm = acctnm;
					rowData.custac = custac;
					rowData.fronsq = fronsq;
					rowData.frondt = frondt;
					rowData.tranAm = tranAm;
					rowData.lncfno = lncfno;
					//rowData.fstatu = fstatu;
					rowData.checkStatus = checkStatus;
					data.push(rowData);
