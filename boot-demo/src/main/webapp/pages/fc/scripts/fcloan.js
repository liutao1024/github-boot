var fcbill = function(){
	...
	var handleFileUpload = function(){
        ...
        var url = Sunline.ajaxPath("fc/fcloandetail");
		...
		$(".table-group-actions").append("<button id='deal' class='btn btn-sm purple table-group-action-submit'><i class='fa fa-rotate-right'></i>&nbsp;放款批量处理</button></div>");
		var sendData = ["cust_id"];
        // 查询按钮
        ...
        $('#deal').click(function(){
			...
			for (i=0;i<rows.length;i++){
				...
				var rowData = {};
				var cust_id = $(row[1]).text();
				rowData.cust_id = cust_id;
				data.push(rowData);
			}
			var input = {};
			input.data = data;
