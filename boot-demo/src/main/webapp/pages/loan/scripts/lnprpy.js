			...
			Sunline.ajaxRouter("fc/pmtweb", {
				...
				if(ret.retCode == "0000"){
					...
					//当实际扣款总额大于借据总金额时，可提前还款
						if(parseFloat(retXmqrtqInfo.resvam) - parseFloat(retPmtralInfo.totalm)<0&&$("#paytyp").val()=='1'){
...
//			$("#lnprpy_form").submit();
//		});
		$("#paytyp").on("change",function(){
		//实际扣款总额减去总金额小于0不允许提前还款
			if(parseFloat(retXmqrtqInfo.resvam) - parseFloat(retPmtralInfo.totalm)<0&&$("#paytyp").val()=='1'){
				$("#submit_btn").addClass("disabled");
			}
			//当实际扣款总额不为0时，默认为借记卡。
			else if(parseFloat(retXmqrtqInfo.resvam)!=0&&$("#paytyp").val()!='1'){
				$("#submit_btn").addClass("disabled");
			}
			else{
