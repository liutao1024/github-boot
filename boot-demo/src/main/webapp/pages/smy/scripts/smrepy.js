var Smrepy = function(){
	...
	var readFile = function(){
		...
		$('#submit').click(function(){
			...
			var input = {};
			input.file = 'unon_nomacth_pay_'; //银联代付文件
			input.file1 = 'bspy_pay_';//包商卡
			input.file2 = 'yschck_pay_';//银盛代付文件
			input.file3 = 'chpy_nomacth_pay_';//chinapay代付文件
			input.file4 = 'unon_nomacth_pay_smy_';
			input.trandt = trandt;
			$("#myModal").modal('show');
			Sunline.ajaxRouter(
	        	"setlle/getSmyPayRecord", 
	        	 input,
	        	"POST",
	            function(redata){
	...
	var handlerTable = function(){
			var url = Sunline.ajaxPath("setlle/qrySmyPayRecord");
			...
			grid.init({
		        ...
		        dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
		            ...
		            "columns" : [{
			            ...
			            {"data": "custno","sortable": false,"searchable": false},
			            ...
			            {"data": "idtfno","sortable": false,"searchable": false},
			            {"data": "custna","sortable": false,"searchable": false},
			            {"data": "mobile","sortable": false,"searchable": false},
			            {"data": "payeca","sortable": false,"searchable": false},
			            {"data": "payena","sortable": false,"searchable": false}
			          /*  {"data": "prodcd","sortable": false,"searchable": false},
			            {"data": "fcbpdt","sortable": false,"searchable": false},
			            {"data": "fcbpsq","sortable": false,"searchable": false},*/
		            ]
		        }
		    });
			$(".table-group-actions").append("<button id='deal_btn' class='btn btn-sm green table-group-action-submit'><i class='fa fa-rotate-right'></i>&nbsp;重新代付</button></div>");
	        ...
	        $('#deal_btn').click(function(){
				...
				Sunline.ajaxRouter(
	                ...
	                function(redata){
	            		...
	            		bootbox.alert("重新代付处理结束，处理["+redata.amount+"]笔中成功["+redata.succAmount+"]笔");
	            		grid.submitFilter();
	            	},
	            	function(redata){
	            		$("#myModal").modal('hide');
