var Smcoqs = function(){
	...
	var readFile = function(){
		...
		$('#submit').click(function(){
			...
			var input = {};
			input.file = 'SMY_SMPA_';
			input.trandt = trandt;
		/*	input.dk_dcqs = dk_dcqs;
			input.dk_hkqs = dk_hkqs;*/
			$("#myModal").modal('show');
			Sunline.ajaxRouter(
	        	"setlle/getSmyCorePayDkRecord", 
	        	 input,
	        	"POST",
	            function(redata){
	...
	var handlerTable = function(){
			var url = Sunline.ajaxPath("setlle/qrySmyCorePayDkRecord");
...
//				}
				...
				for (var i=0;i<rows.length;i++){
					...
					var tranam = $(row[4]).text();
					var fronsq = $(row[6]).text();
					var frondt = $(row[7]).text();
					var custna = $(row[2]).text();
